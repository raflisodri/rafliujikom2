...
@section('content')
                                ...
                                <form action="/transaksi/update/{{ $transaksi->id }}" method="post">
                                                ...
                                                <select name="id_member" id="id_member" class="form-control">
                                                    @foreach ($member as $member)
                                                        <option value="{{ $member->id }}"
                                                            {{ $transaksi->id_member == $member->id ? 'selected' : '' }}>
                                                            {{ $member->nama }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" name="tanggal" id="tanggal" class="form-control"
                                            value="{{ $transaksi->tanggal }}" aria-describedby="helpId">
                                        @error('tanggal')
                                        ...
                                        <div class="col-12">
                                            ...
                                            <div id="sepatu-repeater">
                                                @foreach ($transaksi->detailTransaksi as $index => $detail)
                                                    <div class="sepatu-group mb-3" data-index="{{ $index }}">
                                                        <div class="row">
                                                            <!-- Sepatu -->
                                                            <div class="col-md-4">
                                                                <label for="id_sepatu" class="form-label">Sepatu</label>
                                                                <select name="id_sepatu[]"
                                                                    class="form-control sepatu-select"
                                                                    data-index="{{ $index }}">
                                                                    <option value="">Pilih Sepatu</option>
                                                                    @foreach ($sepatu as $item)
                                                                        <option value="{{ $item->id }}"
                                                                            {{ $detail->id_sepatu == $item->id ? 'selected' : '' }}>
                                                                            {{ $item->merk }} - {{ $item->nama }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            ...
                                                            <div class="col-md-2">
                                                                ...
                                                                <input type="number" name="total[]"
                                                                    class="form-control total" value="{{ $detail->total }}"
                                                                    aria-describedby="helpId" readonly>
                                                            </div>
                                            ...
                                            <div class="row mt-3">
                                                <div class="col-md-4 offset-md-8">
                                                    ...
                                                    <input type="number" name="sub_total" id="subtotal"
                                                        ...
                                                        value="{{ $transaksi->sub_total }}" aria-describedby="helpId"
                                                        readonly>
                                                </div>
                                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let index = {{ $transaksi->detailTransaksi->count() }}; // Initialize index counter
